...
class AjaxController extends Controller
{
    ...
    {
        return array('verify - AddContactInfo');
    }

    //定义的过滤方法
    public function FilterVerify($filterChain)
    {
        // Yii::app()->session['openid'] = "o_68bwFzceyI-wPLceNHdBKTGXD4";
    ...
    //提交抽奖结果
    public function actionSendLuckyGift() {
        $items_id = $_POST['num'];
        $openid = Yii::app()->session['openid'];
        // $items_id = '0';
        // $openid = "oIzS4wXF63E0y3tBeEQjTIzIiIGc";
        $response = array('code'=>-1, 'msg'=>'failed');
        $allow_value = array('0','1','2','3','4');

        // Weixin::sendMsgToUser($openid);
        if( !in_array($items_id, $allow_value) ) {
            $response['code'] = -3;
            $response['msg'] = 'params error'. $items_id ;
        }
        else if( !$lucky_gift = LuckyGift::model()->findByPk($items_id) ) {
            $response['code'] = -4;
            $response['msg'] = 'can not find items';
        }
        else if((int)$lucky_gift['count']<=0) {
            $response['code'] = -5;
            $response['msg'] = 'gift is empty';
        }
        else if( !$at_user = AttentionUser::model()->find("openid =:t_openid", array(":t_openid"=>$openid)) ){
            $response['code'] = -6;
            $response['msg'] = 'user not attention';
        }
        else if( $at_user['is_first']=='yes') {
            $lucky_gift['count'] = (int)$lucky_gift['count'] - 1;
            if($lucky_gift->save()) {
                $at_user['luck_item'] = $items_id;
                $at_user['create_time'] = date("Y-m-d H:i:s");
                $at_user['is_first'] = 'no';
                $at_user->save();
                $response['code'] = 0;
                $response['msg'] = 'ok';
                if($items_id=="0") {
                    ...
                    Weixin::sendMsgToUser($openid, 'account');
                }
            }
        }
        echo CJSON::encode($response);
    ...
    public function actionAddContactInfo() {
        ...
        $items1 = empty($_POST['items1']) ? "" : htmlspecialchars($_POST['items1']);
        $items2 = empty($_POST['items2']) ? "" : htmlspecialchars($_POST['items2']);
        $items1 = 'QQ:'. $items1;

        $response = array('code'=>-1, 'msg'=>'failed');
        $info = new WebContactInfo;
        $info['name'] = $name;
        $info['phone'] = $phone;
        $info['items1'] = $items1;
        $info['items2'] = $items2;
        ...
        Weixin::sendContactMsgToUser($info);
        echo CJSON::encode($response);
    }
