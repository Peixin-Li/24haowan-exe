...
<script src="http://24haowan-cdn.shanyougame.com/public/js/phaser.min.js"></script>
<link rel="stylesheet" type="text/css" href="http://24haowan-cdn.shanyougame.com/FindSingle/main.css?3">

<div id="game-div" class="game-div"></div><!-- 游戏界面 -->
<div id="circle" class="circle"></div><!-- 圆圈 -->
<div id="startButton" class="start-button"></div><!-- 开始按钮 -->
<div id="rankButton"></div><!-- 排行榜按钮 -->
...
<div id="pause-div" class="pause-div">
	<div class="pause-container">
		...
		<div class="pause-text-container">
			<div id="pauseText" class="pause-text">别再说自己是单身狗了，你这个年纪，狗已经死了</div>
		</div>
...
(function () {
	...
	// 暂停的话和狗头
	var pauseResource = [
		{"text": "别再说自己是单身狗了，你这个年纪，狗已经死了。", "pic": "http://24haowan-cdn.shanyougame.com/FindSingle/assets/pause-1.png"},
		{"text": "单身狗也是狗，秀恩爱属于虐狗行为。", "pic": "http://24haowan-cdn.shanyougame.com/FindSingle/assets/pause-2.png"},
		{"text": "“你喜欢猫呢还是狗呢?”</br>“喜欢狗”</br>“汪汪~”", "pic": "http://24haowan-cdn.shanyougame.com/FindSingle/assets/pause-3.png"},
		{"text": "跑到喜欢的女生面前躺下对她说“同学，你男朋友掉地上了。”", "pic": "http://24haowan-cdn.shanyougame.com/FindSingle/assets/pause-4.png"},
		{"text": "“诶同学，我怎么越看你越像我下一任女友耶。”", "pic": "http://24haowan-cdn.shanyougame.com/FindSingle/assets/pause-5.png"}
	];
		...
		for (var j = 0; j < personsNum.y; j++) {
			for (var i = 0; i < personsNum.x; i++) {
				...
				if (i == singleLocation.offsetX && j == singleLocation.offsetY) {
					...
					// 创建单身狗
					single = persons.create(x, y, 'dogAnimation');
					single.frame = random_face;
					single.animations.add('found', [random_face, 1, random_face, 1], 10, false);
				} else {
					// 随机抽取一个情侣
	...
	function initGame() {
		...
		game.States.boot = function () {
			this.preload = function () {
				...
				// 加载加载时的素材
				// game.load.image('progress-empty', 'http://24haowan-cdn.shanyougame.com/FindSingle/assets/'+device+'/progress-empty.png');
				// game.load.image('progress-fill', 'http://24haowan-cdn.shanyougame.com/FindSingle/assets/'+device+'/progress-fill.png');
				// game.load.image('pic-start', 'http://24haowan-cdn.shanyougame.com/FindSingle/assets/'+device+'/pic-start.png');
				game.load.atlasJSONHash('texture-loading', 'http://24haowan-cdn.shanyougame.com/FindSingle/assets/'+device+'/texture-loading.png', 'http://24haowan-cdn.shanyougame.com/FindSingle/assets/'+device+'/texture-loading.json');
		...
		game.States.preload = function () {
			this.preload = function () {
				...
				var pic_start = game.add.image(0, 0, 'texture-loading', 'pic-start.png');
				...
				if (device == "mobile") {
					...
					progress_empty = game.add.sprite((playground.width-326)/2, (playground.height-70)/2+40, 'texture-loading', 'progress-empty.png');
					...
					progress_fill = game.add.sprite((playground.width-320)/2, (playground.height-60)/2+40, 'texture-loading', 'progress-fill.png');
					...
					progress_empty = game.add.sprite((playground.width-486)/2, (playground.height-110)/2+60, 'texture-loading', 'progress-empty.png');
					...
					progress_fill = game.add.sprite((playground.width-480)/2, (playground.height-90)/2+60, 'texture-loading', 'progress-fill.png');
				...
				// 加载其他资源
				game.load.audio('sound_win', 'http://24haowan-cdn.shanyougame.com/FindSingle/assets/audio/win.wav');
				game.load.audio('sound_bg', 'http://24haowan-cdn.shanyougame.com/FindSingle/assets/audio/bg.mp3');
				game.load.image('icon-doge', 'http://24haowan-cdn.shanyougame.com/FindSingle/assets/icon-doge.png');
				// game.load.image('bg-start', 'http://24haowan-cdn.shanyougame.com/FindSingle/assets/'+device+'/bg-start.png');
				// game.load.image('icon-countDown', 'http://24haowan-cdn.shanyougame.com/FindSingle/assets/'+device+'/bg-countDown.png');
				// game.load.image('bg-score', 'http://24haowan-cdn.shanyougame.com/FindSingle/assets/'+device+'/bg-score.png');
				game.load.atlasJSONHash('texture-play', 'http://24haowan-cdn.shanyougame.com/FindSingle/assets/'+device+'/texture-play.png', 'http://24haowan-cdn.shanyougame.com/FindSingle/assets/'+device+'/texture-play.json');

				if (device == "mobile") {
					game.load.spritesheet('persons', 'http://24haowan-cdn.shanyougame.com/FindSingle/assets/mobile/persons.png', 150, 300);
					game.load.spritesheet('dogAnimation', 'http://24haowan-cdn.shanyougame.com/FindSingle/assets/mobile/dogAnimation.png', 150, 300);
				} else {
					game.load.spritesheet('persons', 'http://24haowan-cdn.shanyougame.com/FindSingle/assets/pad/persons.png', 225, 450);
					game.load.spritesheet('dogAnimation', 'http://24haowan-cdn.shanyougame.com/FindSingle/assets/pad/dogAnimation.png', 225, 450);
				}
			};
			this.create = function() {
				game.state.start('create');
		...
		game.States.create = function () {
			this.create = function () {
				...
				// 开始按钮
				$("#startButton").css("display", "block").css("left", (windowInnerWidth-240)/2 - 10).css("top", windowInnerHeight*3/4);
				...
				//排行榜按钮
				$("#rankButton").css("display", "block").css("left", (windowInnerWidth-240)/2 + 130).css("top", windowInnerHeight*3/4);
				$("#rankButton").on("tap", function(e) {
					e.preventDefault();
					e.stopPropagation();
					showRankBox();
				});
