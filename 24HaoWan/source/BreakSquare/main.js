...
var currentLevel = parseInt(stage) || 1; //关卡
var score = parseInt(last_score) || 0; //分数
...
game.States.preload = function(){
	this.preload = function(){
		...
		//游戏资源 平台小球道具等
		game.load.atlasJSONArray('sprite','http://24haowan-cdn.shanyougame.com/BreakSquare/' + device + '/sprite.png','http://24haowan-cdn.shanyougame.com/BreakSquare/' + device + '/sprite.json');

		game.load.image('paddle','http://24haowan-cdn.shanyougame.com/BreakSquare/' + device + '/paddle.png');
		//游戏背景
		game.load.image('bg','http://24haowan-cdn.shanyougame.com/BreakSquare/' + device + '/background.png');

		//方块
		game.load.atlasJSONArray('pixels', 'http://24haowan-cdn.shanyougame.com/BreakSquare/' + device + '/pixels.png', 'http://24haowan-cdn.shanyougame.com/BreakSquare/' + device + '/pixels.json');
		//音效
		game.load.audio('btn_music','http://24haowan-cdn.shanyougame.com/BreakSquare/music/btn_music.mp3');
		game.load.audio('levelUp_music','http://24haowan-cdn.shanyougame.com/BreakSquare/music/levelUp.mp3');
		game.load.audio('gameOver_music','http://24haowan-cdn.shanyougame.com/BreakSquare/music/gameOver.mp3');
		game.load.audio('collide_music','http://24haowan-cdn.shanyougame.com/BreakSquare/music/collide.mp3');
		game.load.audio('bg_music','http://24haowan-cdn.shanyougame.com/BreakSquare/music/bg_music.mp3');
	};
	this.create = function(){
		//加载音乐
...
game.States.play = function(){
	this.create = function(){
		...
		levelLabel.text = '关卡:'+currentLevel;
	...
	this.ballHitSquare = function(_ball, _pixel){
		// 撞到炎魔加速
		if (_pixel.type == 3) {
			var vx = _ball.body.velocity.x;
			var vy = _ball.body.velocity.y;
			_ball.body.velocity.x = vx * 1.5;
			_ball.body.velocity.y = vy * 1.5;
			slowVelocity(vx,vy,_ball);
		} 
		...
		if(squareGroup.countLiving() == 0) {
			this.pix.init(++currentLevel);
			setGameScore(score,getBadge(score),resetShareText(),currentLevel);
			levelUp_music.play();
			this.bal.resetBall();
...
Paddle.prototype.init = function(){
	paddle = game.add.sprite(canvasWidth/2,canvasHeight - 150,'paddle');
	paddle.len = 1;
	paddle.anchor.setTo(0.5);
	game.physics.arcade.enable(paddle,false);
	paddle.body.setSize(game.cache.getImage('paddle').width,game.cache.getImage('paddle').height-20);
...
Ball.prototype.init = function(){
	if(this.ballNum == 0) {
		var distance = device == 'mobile'?30:55;
		var one = ball.create(paddle.x,paddle.y-distance,'sprite','ball.png');
		this.ballNum++;
...
Ball.prototype.splitBall = function() {
	if(this.ballNum == 1 && this.isSplit == false) {
		...
		var one = ball.create(two.x,two.y,'sprite','ball.png');
		this.ballNum++;
		one.anchor.setTo(0.5);
		one.scale.setTo(0.8);
		one.checkWorldBounds = true;
...
Ball.prototype.ballLost = function(e){
	...
	if(this.ballNum <= 0) {
		...
		currentLevel = 1;
		score = 0;
		setGameScore(score,getBadge(score),resetShareText(),currentLevel,false);
	}
};
//重置小球
...
var Pixel = function(){
	...
	this.levelSetting_arr = [
		{"x":7, "y":3}, // 1
		{"x":7, "y":3, "m4":[2, 5]}, // 2
		{"x":9, "y":4, "m2":[2, 5], "m4":[2, 5]}, // 3-4
		{"x":9, "y":5, "m2":[5, 10], "m3":[3, 5], "m4":[2, 5]}, // 5-6
		{"x":11, "y":5, "m2":[10, 15], "m3":[5, 10], "m4":[2, 5]}, // 7-8
		{"x":11, "y":5, "m1":[5, 10], "m2":[10, 15], "m3":[5, 10], "m4":[2, 5]}, // 9
		{"x":11, "y":6, "m1":[15, 20], "m2":[15, 20], "m3":[10, 15], "m4":[5, 8]}, // 10及以后
	]; //五种关卡类型
	this.levelType = 0;
	this.map = [];
	...
	//中间部分不变
	help_pic = game.add.sprite(this.numX*6.5,this.numY*((device == "pad") ? 2.88 : 3),'sprite','helpMe.png');
...
Pixel.prototype.initMap = function (level) {
	...
	if (level < 2) { // 1
		...
		this.createArr(this.levelSetting_arr[6]);
		this.levelType = 6;
	}
}
...
Pixel.prototype.prop = function(obj) {
	if(obj.type == 5) { // 宝箱怪
		//下方平板变短变长的道具
		createProp(obj,((Math.random() < 0.3) ? ((Math.random() > 0.5) ? 1 : 2) : 0));
	} else if(obj.type == 4) {
		// 一个小球变两个的道具
		createProp(obj,3);
	}
...
//缓慢减速
function slowVelocity(vx,vy,_ball) {
	var time = 50;
	timer = setInterval(function(){
		subX = Math.abs(vx/4000*time);
		subY = Math.abs(vy/4000*time);
		_ball.body.velocity.x += (_ball.body.velocity.x>0?-subX:+subX);
		_ball.body.velocity.y += (_ball.body.velocity.y>0?-subY:+subY);
		if(Math.abs(_ball.body.velocity.x)<Math.abs(vx) || Math.abs(_ball.body.velocity.y)<Math.abs(vy)) {
			_ball.body.velocity.x = _ball.body.velocity.x>0?Math.abs(vx):Math.abs(vx)*-1;
			_ball.body.velocity.y = _ball.body.velocity.y>0?Math.abs(vy):Math.abs(vy)*-1;
			clearInterval(timer);
		}
	},time);
}
...
var createProp = function(obj,type) {
	if(type >= 1 && type <= 3) {
		if(type == 3) {
			var one = propGroup.create(obj.x,obj.y,'sprite','splitProp.png');
		}else {
			var one = propGroup.create(obj.x,obj.y,'sprite','prop.png');
		}
		one.anchor.setTo(0.5);
		game.physics.arcade.enable(one,true);
		one.checkWorldBounds = true;
