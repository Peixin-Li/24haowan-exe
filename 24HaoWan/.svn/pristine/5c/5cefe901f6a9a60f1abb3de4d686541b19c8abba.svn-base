...
class User extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('user_id, openid, name, sex, province, city, country, headimgurl, unionid, from,create_time,last_update_time, last_login_time, last_login_ip, enable', 'safe', 'on'=>'search'),
		);
	}
	...
	//获取用户信息，本地数据库会存储用户信息
	public static function getLocalUserInfo($code, $from="weixin") {
		...
		if(!empty($web_access_token_arr['openid']) ) {
            ...
            if(empty($local_user_info['openid']) || ($expire_time < time()) ) {       //当前用户信息6小时个更新一次
                if($weixin_user_info = Weixin::getUserInfo($web_access_token, $openid)) {
                	...
                	$local_user_info['headimgurl'] = preg_replace('/\d+$/', '64', $weixin_user_info['headimgurl']);
