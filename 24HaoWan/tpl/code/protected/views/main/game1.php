...
<body>
	...
	<div id="bear-over-box">
		<div class="show-box">
			<div id="top-text">点击右上角，炫耀我找到的熊本熊</div>
...
<script type="text/javascript">
	$('#tip').on('tap',function(){
		...
		} else if($('.page_2').css('display') == 'block') {
...
// (function () {
	...
	//提交分数
	function setGameScore_bear(score,badge,obj) {
		obj.title = obj.title.replace('{score}',score);
		if(score<=10) {
			obj.title += ',我开始怀疑自己有点弱视...';
		} else if(score<=25){
			obj.title += ',感知能力是天生的辣~';
		} else if(score<=40) {
			obj.title += ',我的眼力和福尔摩斯有的一拼!';
		} else {
			obj.title += ',眼力太好！FBI给我发了面试邀请...';
		}
		wxReady(obj);
	}
		...
		game.States.play = function () {
			this.create = function () {
				...
				if (device == "mobile") {
					...
					// iconDoge.width = 38;
					// iconDoge.height = 38;
					dogeText = game.add.text(100, 30, '0', {fontSize: '38px', fill:"#FFFFFF"});
...
// })();
	...
	function wxReady(obj) {
		obj = obj || {};
		var text = obj.title || '要出发·寻找熊本部长';
		var imgUrl = obj.imgUrl || 'http://24haowan-cdn.shanyougame.com/tourist/assets/icon-bear.png';
		var desc = obj.desc || '';
		wx.ready(function () {
		// 在这里调用 API
		//分享朋友圈
